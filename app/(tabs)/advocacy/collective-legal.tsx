...
export default function CollectiveLegal() {
  ...
  React.useEffect(() => { (async () => { const saved = await getCachedJSON<Report[]>('collective_reports'); if (saved) setReports(saved); })(); }, []);
  React.useEffect(() => { setCachedJSON('collective_reports', reports); }, [reports]);

  const submit = async () => {
    const r: Report = { id: String(Date.now()), type, province, details, createdAt: Date.now() };
    setReports((prev) => [r, ...prev]);
    fsAddViolationReport?.(r).catch(()=>{});
    setDetails('');
    Alert.alert('Submitted', 'Report added anonymously. Aggregated counts help build leverage.');
  };

  const byType = reports.reduce((acc: Record<string, number>, r) => { acc[r.type] = (acc[r.type]||0)+1; return acc; }, {} as any);
  ...
  return (
    <ScrollView style={s.container} contentContainerStyle={{ padding: 16 }}>
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>
        {TYPES.map(t => (
          <Pressable key={t} onPress={() => setType(t)} style={{ borderWidth: 1, borderColor: palette.muted, borderRadius: 16, paddingHorizontal: 12, paddingVertical: 6, backgroundColor: type===t?palette.primary:'transparent' }}><Text style={{ color: type===t?palette.onPrimary:palette.text }}>{t}</Text></Pressable>
        ))}
      </View>
      <Text style={s.label}>Province (optional)</Text>
      <TextInput style={s.input} value={province} onChangeText={setProvince} placeholder="ON, BC, ..." />
      <Text style={s.label}>Details (optional)</Text>
      <TextInput style={[s.input,{ minHeight: 100 }]} value={details} onChangeText={setDetails} multiline placeholder="Short summary; do not include identifiers." />
      <Pressable onPress={submit} style={s.button}><Text style={s.buttonText}>Submit report</Text></Pressable>

...
const SECTIONS = [
  { title: 'Human Rights & Duty to Accommodate', items: [
    { label: 'What is the duty to accommodate?', url: 'https://www.chrc-ccdp.gc.ca/en/resources/what-duty-accommodate' },
    { label: 'Ontario Human Rights Commission', url: 'https://www.ohrc.on.ca/' },
  ]},
  { title: 'Accessibility Laws', items: [
    ...
    <ScrollView style={s.container} contentContainerStyle={{ padding: 16 }}>
      ...
      {SECTIONS.map(sec => (
        <View key={sec.title} style={s.card}>
          ...
          {sec.items.map(it => (
            <Pressable key={it.label} onPress={()=>open(it.url)} accessibilityRole="link">

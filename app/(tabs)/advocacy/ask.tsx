...
function Inner() {
  ...
  const submit = () => {
    ...
    if (ok) {
      ...
      Alert.alert("Slow down", "Please wait a few seconds before posting again.");
    }
  };

  return (
    <View style={styles.container} accessibilityLabel="Ask an Advocate form" accessible>
      <Text ref={titleRef} accessibilityRole="header" style={styles.title} maxFontSizeMultiplier={MAX_FONT_SCALE}>Ask an Advocate</Text>
      <Text style={styles.subtitle}>Briefly describe your issue. Do not include personal identifiers.</Text>

      <Text style={styles.label}>Category</Text>
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>
        {topicChannels.map((c) => (
          <Pressable key={c.id} onPress={() => setChannelId(c.id)} style={[styles.chip, channelId===c.id && styles.chipActive]} accessibilityRole="button">
            <Text style={[styles.chipText, channelId===c.id && styles.chipTextActive]}>{c.title}</Text>
          </Pressable>
        ))}
      </View>
      ...
      <TextInput style={[styles.input, { minHeight: 100 }]} value={details} onChangeText={setDetails} multiline placeholder="What happened? What help do you need?" />
...
function createStyles(palette: ReturnType<typeof useAppPalette>) {
  return StyleSheet.create({
    ...
    label: { color: palette.text, opacity: 0.95, marginTop: 8, marginBottom: 4 },
    input: { borderWidth: 1, borderColor: palette.muted, borderRadius: 8, padding: 10, color: palette.text },
    button: { marginTop: 12, backgroundColor: palette.primary, paddingVertical: 10, borderRadius: 8, alignItems: 'center' },

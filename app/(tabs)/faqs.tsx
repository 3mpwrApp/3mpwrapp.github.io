...
export default function FaqsScreen() {
  ...
  return (
    <View style={styles.container}>
      <Text ref={titleRef} style={styles.title} accessibilityRole="header" maxFontSizeMultiplier={MAX_FONT_SCALE}>FAQs</Text>
      ...
      <View style={{ marginBottom: 8 }}>
        <TextInput style={styles.input} value={qText} onChangeText={setQText} placeholder="Question" placeholderTextColor={palette.text} />
        <TextInput style={[styles.input, { minHeight: 80 }]} value={aText} onChangeText={setAText} placeholder="Answer" placeholderTextColor={palette.text} multiline />
        <Pressable
          disabled={!qText.trim() || !aText.trim()}
          onPress={async () => {
            const item = { id: `faq-${Date.now()}`, q: qText.trim(), a: aText.trim() };
          ...
          style={({ pressed }) => [styles.button, (!qText.trim() || !aText.trim() || pressed) && { opacity: 0.7 }]}
        >
...
function createStyles(palette: ReturnType<typeof useAppPalette>, factor: number) {
  return StyleSheet.create({
    ...
    item: { paddingVertical: 10, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: palette.muted },

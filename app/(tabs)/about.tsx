...
import { View, Text, StyleSheet, TextInput, Pressable, Linking, Alert } from "react-native";
...
export default function AboutScreen() {
  ...
  const sendEmail = async () => {
    ...
    if (!supported) return Alert.alert("Email not configured", `Please email ${EMAIL}`);
    await Linking.openURL(url);
  };

  return (
    <View style={styles.container}>
      <Text ref={titleRef} style={styles.title} accessibilityRole="header" maxFontSizeMultiplier={MAX_FONT_SCALE}>About & Contact</Text>
      ...
      <TextInput style={[styles.input, { minHeight: 120 }]} value={message} onChangeText={setMessage} placeholder="Your message" placeholderTextColor={palette.text} accessibilityLabel="Message" multiline />
      <Pressable style={({ pressed }) => [styles.button, pressed && { opacity: 0.9 }]} onPress={sendEmail} accessibilityRole="button" accessibilityLabel="Send email">

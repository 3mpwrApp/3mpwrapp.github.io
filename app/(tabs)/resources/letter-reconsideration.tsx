...
export default function ReconsiderationLetter() {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ padding: 20 }}>
      ...
      <Pressable accessibilityRole="button" accessibilityLabel="Share letter" style={styles.button} onPress={() => { logEvent('letter_share', { type: 'reconsideration' }); Share.share({ message: preview, title: "Reconsideration Letter" }); }}>
        <Text style={styles.buttonText}>Share</Text>
      </Pressable>
      <Pressable accessibilityRole="button" accessibilityLabel="Export PDF" style={[styles.button, { marginTop: 8 }]} onPress={async () => {
        try { const mod = await import("expo-print"); const html = `<pre style=\"font-family: system-ui, -apple-system, Segoe UI, Roboto, Arial; white-space: pre-wrap;\">${preview.replace(/&/g, "&amp;").replace(/</g, "&lt;")}</pre>`; const { uri } = await mod.printToFileAsync({ html }); await Share.share({ url: uri, title: "Reconsideration Letter" }); }
      ...
      <Pressable accessibilityRole="button" accessibilityLabel="Copy letter" style={[styles.button, { marginTop: 8 }]} onPress={async () => {
        try {

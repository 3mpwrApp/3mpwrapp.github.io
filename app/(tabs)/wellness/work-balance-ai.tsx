...
export default function WorkBalanceAI() {
  ...
  const generate = async () => {
    ...
    const avgSleep = (() => {
      const hours = slp.slice(0, 7).map((e) => parseFloat(e.sleepHours || "0")).filter((n)=>!isNaN(n));
  ...
  return (
    <ScrollView style={s.container} contentContainerStyle={{ padding: 16 }}>
      <Text ref={titleRef} accessibilityRole="header" style={s.title} maxFontSizeMultiplier={MAX_FONT_SCALE}>Wellness + Work Balance AI</Text>
      <Text style={s.subtitle}>Tracks pain, sleep/energy, and mood to suggest when to rest, when to work, and when to advocate.</Text>
      <Text style={s.label}>Mood</Text>
      <View style={{ flexDirection: 'row', gap: 8, marginBottom: 8 }}>
        {(["low","ok","high"] as Mood[]).map((m) => (
          <Pressable key={m} onPress={() => setMood(m)} style={[s.chip, mood===m && s.chipActive]} accessibilityRole="button"><Text style={[s.chipText, mood===m && s.chipTextActive]}>{m}</Text></Pressable>
        ))}
      </View>
      <Text style={s.label}>Notes (optional)</Text>
      <TextInput style={s.input} value={notes} onChangeText={setNotes} placeholder="Important errands, appointments, deadlines" />
      <Pressable onPress={generate} style={s.button}><Text style={s.buttonText}>Plan my day</Text></Pressable>
      {!!plan && (
        <View style={s.box}>
          <Text style={{ color: palette.text }}>{plan}</Text>
          <View style={{ height: 8 }} />
          <Pressable onPress={async () => { try { const mod = await import('expo-clipboard'); await mod.setStringAsync(plan); Alert.alert('Copied','Plan copied to clipboard.'); } catch {} }} style={s.button}><Text style={s.buttonText}>Copy</Text></Pressable>
        </View>
      )}
    </ScrollView>
  );

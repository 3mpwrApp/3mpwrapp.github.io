...
export default function SymptomTracker() {
  ...
  // Persist on change
  React.useEffect(() => { setCachedJSON("wellness_symptom_entries", entries); }, [entries]);
  ...
  const report = React.useMemo(() => {
    ...
    const pains = filtered.map((e) => parseFloat(e.pain || "0")).filter((n) => !isNaN(n));
    ...
    lines.push("- Functional impact reported across dates above (consider accommodations and pacing).");
    lines.push("- Correlate high-pain days with missed work/ADLs and provide medical notes as available.");
    return lines.join("\n");
  }, [filtered]);
  ...
  const body = (
    <ScrollView style={styles.container} contentContainerStyle={{ padding: 16 }}>
        ...
        {filtered.slice(0, 10).reverse().map((e) => {
          ...
          return (
            <View key={e.id} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 4 }}>
              ...
              <Text style={{ color: palette.text, marginLeft: 6 }}>{e.pain || "?"}</Text>
            </View>
          );
        })}
      </View>
      ...
      <View style={styles.preview}><Text style={styles.previewText}>{report}</Text></View>

      <Pressable style={styles.button} onPress={() => { try { require("../../../services/analytics").logEvent?.('tracker_share', { kind: 'symptom' }); } catch {}; Share.share({ message: report, title: "Symptom & Pain Report" }).catch(() => {}) }}>
        <Text style={styles.buttonText}>Share</Text>
      </Pressable>
      <Pressable style={[styles.button, { marginTop: 8 }]} onPress={async () => {
        try { const mod = await import("expo-clipboard"); await mod.setStringAsync(report); Alert.alert("Copied", "Report copied to clipboard."); }

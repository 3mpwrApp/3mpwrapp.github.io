# WSIAT Deep Dive Analysis Report
## Super Detective Mode 🕵️ - 98,992 Decisions

**Generated:** 2026-04-30  
**Total Decisions:** 99,036  
**Analysis Depth:** 9 advanced pattern categories  
**Data Source:** [WSIAT Open Data Portal](https://www.wsiat.ca/en/home/opendata_decisions.html) - Official CSV export

**Related Resources:**
- [Evidence-Based Guides](/guides/) - Practical appeal strategies based on this analysis
- [Interactive Network Visualization](/connecting-the-dots-wsiat-keyword-network.html) - 16 nodes, 50 links
- [WSIAT vs BC WCAT Comparison](/research/tribunal-transparency/wsiat-vs-bc-wcat-transparency-divide/) - Transparency analysis
- [Research Tools](/research/) - Interactive data explorer

---

## 🔗 1. Keyword Co-Occurrence Patterns

### Top 10 Keyword Pairs (Issues That Appear Together)

| Rank | Keyword 1 | Keyword 2 | Co-Occurrences | % of Decisions |
|------|-----------|-----------|----------------|----------------|
| 1 | NEL | Permanent Impairment | 11,516 | 11.63% |
| 2 | LOE | Loss of Earnings | 9,167 | 9.26% |
| 3 | LOE | NEL | 3,546 | 3.58% |
| 4 | Pre-existing | SIEF | 3,281 | 3.31% |
| 5 | Loss of Earnings | NEL | 3,034 | 3.06% |
| 6 | LOE | Permanent Impairment | 2,292 | 2.31% |
| 7 | Chronic Pain | NEL | 2,101 | 2.12% |
| 8 | Loss of Earnings | Permanent Impairment | 2,050 | 2.07% |
| 9 | FEL | NEL | 1,781 | 1.80% |
| 10 | Chronic Pain | Permanent Impairment | 1,447 | 1.46% |

**Insight:** These pairs indicate common appeal combinations. For example, if NEL + Chronic Pain appear together frequently, create a guide addressing both.

---

## 📈 2. Temporal Evolution (40 Years)

### Issue Trends by Decade

**NEL:**
- 1980s: 0
- 1990s: 901
- 2000s: 4568
- 2010s: 7317
- 2020s: 2741

**LOE:**
- 1980s: 0
- 1990s: 0
- 2000s: 1887
- 2010s: 5494
- 2020s: 3250

**FEL:**
- 1980s: 0
- 1990s: 877
- 2000s: 3120
- 2010s: 794
- 2020s: 112

**SIEF:**
- 1980s: 52
- 1990s: 841
- 2000s: 847
- 2010s: 1738
- 2020s: 1123

**Initial Entitlement:**
- 1980s: 40
- 1990s: 132
- 2000s: 112
- 2010s: 254
- 2020s: 691


---

## 👨‍⚖️ 3. Vice-Chair Specialization

### Top 10 Specialists (Vice-Chairs with >30% Focus on Specific Issues)

| Rank | Vice-Chair | Specialization | Cases | % of Their Work |
|------|------------|----------------|-------|-----------------|
| 1 | M. Gordon | LOE | 59 | 48.8% |
| 2 | M. Gordon | Loss of Earnings | 59 | 48.8% |
| 3 | A. Durette | LOE | 61 | 46.6% |
| 4 | A. Durette | Loss of Earnings | 60 | 45.8% |
| 5 | C. D'Angelo | LOE | 59 | 44.4% |
| 6 | C. D'Angelo | Loss of Earnings | 59 | 44.4% |
| 7 | P. Ahlfeld | LOE | 69 | 43.7% |
| 8 | P. Ahlfeld | Loss of Earnings | 68 | 43.0% |
| 9 | M. Falcone | NEL | 51 | 41.5% |
| 10 | C. Zehr (PT) | LOE | 127 | 40.6% |

**Insight:** These vice-chairs handle disproportionately high volumes of specific issues, suggesting expertise.

---

## 🦴 4. Body Part Injury Patterns

### Most Common Injuries

| Rank | Body Part | Cases | % of Decisions |
|------|-----------|-------|----------------|
| 1 | Back | 13,407 | 13.54% |
| 2 | Shoulder | 5,295 | 5.35% |
| 3 | Neck | 3,535 | 3.57% |
| 4 | Knee | 3,162 | 3.19% |
| 5 | Hand | 2,785 | 2.81% |
| 6 | Foot | 1,462 | 1.48% |
| 7 | Head | 1,198 | 1.21% |
| 8 | Arm | 1,050 | 1.06% |
| 9 | Elbow | 1,001 | 1.01% |
| 10 | Leg | 752 | 0.76% |

---

## 🏥 5. Medical Specialist Mentions

### Top 5 Specialists Referenced

| Rank | Specialist Type | Mentions | % of Decisions |
|------|----------------|----------|----------------|
| 1 | Surgeon | 195 | 0.20% |
| 2 | Family Doctor | 119 | 0.12% |
| 3 | Psychiatrist | 44 | 0.04% |
| 4 | Psychologist | 29 | 0.03% |
| 5 | Occupational Medicine | 21 | 0.02% |

---

## 📜 6. WSIB Policy Citations

### Most Cited Policies

| Rank | Policy/Section | Citations | % of Decisions |
|------|---------------|-----------|----------------|
| 1 | Section 31 | 42 | 0.04% |
| 2 | Section 147 | 31 | 0.03% |
| 3 | Section 43 | 15 | 0.02% |
| 4 | Section 44 | 5 | 0.01% |
| 5 | Section 45 | 4 | 0.00% |

---

## 🧩 7. Decision Complexity Distribution

| Complexity Level | Count | % of Total |
|------------------|-------|------------|
| Simple (1 issue) | 19,656 | 19.85% |
| Moderate (2-3 issues) | 22,787 | 23.01% |
| Complex (4-5 issues) | 2,787 | 2.81% |
| Highly Complex (6+ issues) | 79 | 0.08% |

### Top 5 Most Complex Decisions

1. **Decision 1309/20/20** - 7 issues: NEL, LOE, SIEF, Pre-existing, Permanent Impairment, Loss of Earnings, PTSD
2. **Decision 1273/21/21** - 7 issues: NEL, LOE, SIEF, Initial Entitlement, Pre-existing, Permanent Impairment, Loss of Earnings
3. **Decision 1792/21/21** - 7 issues: NEL, LOE, Chronic Pain, Pre-existing, Permanent Impairment, Loss of Earnings, CPD
4. **Decision 861/22/22** - 7 issues: NEL, LOE, Initial Entitlement, Chronic Pain, Permanent Impairment, Loss of Earnings, CPD
5. **Decision 106/22/22** - 7 issues: NEL, LOE, Chronic Pain, Pre-existing, Permanent Impairment, Loss of Earnings, Mental Health

---

## ⚖️ 8. Outcome Indicators

### Detected Outcomes

| Outcome | Mentions | % of Decisions |
|---------|----------|----------------|
| Allowed | 13,356 | 13.49% |
| Dismissed | 5,448 | 5.50% |
| Remitted | 2,141 | 2.16% |
| Varied | 22 | 0.02% |

**Note:** These are text-based indicators, not definitive outcomes. Many decisions don't explicitly state outcomes in keywords/summaries.

---

## 🕸️ 9. Keyword Network Visualization

**Network Stats:**
- **Nodes:** 16 keywords
- **Links:** 50 co-occurrence connections
- **Strongest Link:** NEL ↔ Permanent Impairment (11,516 cases)

**Visualization File:** `/data/visualizations/wsiat-keyword-network.json`

---

## 🎯 Key Insights for Knowledge Base

### 1. Create Multi-Issue Guides

Top co-occurring pairs suggest users need guides addressing:
1. **NEL + Permanent Impairment** (11,516 cases)
2. **LOE + Loss of Earnings** (9,167 cases)
3. **LOE + NEL** (3,546 cases)
4. **Pre-existing + SIEF** (3,281 cases)
5. **Loss of Earnings + NEL** (3,034 cases)

### 2. Body Part-Specific Templates

Most common injuries warrant specialized templates:
1. **Back** (13,407 cases)
2. **Shoulder** (5,295 cases)
3. **Neck** (3,535 cases)
4. **Knee** (3,162 cases)
5. **Hand** (2,785 cases)

### 3. Specialist Referral Guide

Create "Which Specialist Do I Need?" guide featuring:
1. **Surgeon** (mentioned in 0.20% of decisions)
2. **Family Doctor** (mentioned in 0.12% of decisions)
3. **Psychiatrist** (mentioned in 0.04% of decisions)
4. **Psychologist** (mentioned in 0.03% of decisions)
5. **Occupational Medicine** (mentioned in 0.02% of decisions)

### 4. Policy Citation Patterns

Most-cited policies need dedicated explainers:
1. **Section 31** (42 citations)
2. **Section 147** (31 citations)
3. **Section 43** (15 citations)
4. **Section 44** (5 citations)
5. **Section 45** (4 citations)

---

## 📊 Data Files

All analysis results saved to:
- `/data/tribunal-decisions/wsiat/deep-analysis/keyword-cooccurrence.json`
- `/data/tribunal-decisions/wsiat/deep-analysis/temporal-evolution.json`
- `/data/tribunal-decisions/wsiat/deep-analysis/vice-chair-specialization.json`
- `/data/tribunal-decisions/wsiat/deep-analysis/body-part-patterns.json`
- `/data/tribunal-decisions/wsiat/deep-analysis/medical-specialists.json`
- `/data/tribunal-decisions/wsiat/deep-analysis/policy-citations.json`
- `/data/tribunal-decisions/wsiat/deep-analysis/decision-complexity.json`
- `/data/tribunal-decisions/wsiat/deep-analysis/outcome-indicators.json`
- `/data/visualizations/wsiat-keyword-network.json`

---

**Generated by:** WSIAT Deep Dive Analysis v2.0  
**License:** Open Data (CC BY 4.0)

...
export function generateCanadianHolidays(year: number): Event[] {
  const events: Event[] = [];
  // New Year's Day
  events.push({ id: `holiday-${year}-01-01`, title: "New Year's Day", description: "Federal holiday", date: ymd(year, 0, 1), location: "Canada" });
  // Family Day (most provinces) - 3rd Monday in Feb (advisory)
  const familyDay = nthWeekdayOfMonth(year, 1, 1, 3);
  events.push({ id: `holiday-${year}-${pad2(familyDay.getMonth() + 1)}-${pad2(familyDay.getDate())}-family`, title: "Family Day (Most Provinces)", description: "Observed in many provinces", date: ymd(year, familyDay.getMonth(), familyDay.getDate()), location: "Canada" });
  // Good Friday (Easter Sunday - 2 days)
  const easter = easterSunday(year);
  const goodFriday = new Date(easter);
  goodFriday.setDate(goodFriday.getDate() - 2);
  events.push({ id: `holiday-${year}-${pad2(goodFriday.getMonth() + 1)}-${pad2(goodFriday.getDate())}-goodfriday`, title: "Good Friday", description: "Federal holiday", date: ymd(year, goodFriday.getMonth(), goodFriday.getDate()), location: "Canada" });
  // Victoria Day (Monday preceding May 25)
  const victoria = mondayPrecedingMay25(year);
  events.push({ id: `holiday-${year}-${pad2(victoria.getMonth() + 1)}-${pad2(victoria.getDate())}-victoria`, title: "Victoria Day", description: "Federal holiday", date: ymd(year, victoria.getMonth(), victoria.getDate()), location: "Canada" });
  // Canada Day
  events.push({ id: `holiday-${year}-07-01`, title: "Canada Day", description: "Federal holiday", date: ymd(year, 6, 1), location: "Canada" });
  // Labour Day (1st Monday in September)
  const labour = nthWeekdayOfMonth(year, 8, 1, 1);
  events.push({ id: `holiday-${year}-${pad2(labour.getMonth() + 1)}-${pad2(labour.getDate())}-labour`, title: "Labour Day", description: "Federal holiday", date: ymd(year, labour.getMonth(), labour.getDate()), location: "Canada" });
  // Thanksgiving (2nd Monday in October)
  const thanksgiving = nthWeekdayOfMonth(year, 9, 1, 2);
  events.push({ id: `holiday-${year}-${pad2(thanksgiving.getMonth() + 1)}-${pad2(thanksgiving.getDate())}-thanks`, title: "Thanksgiving", description: "Federal holiday", date: ymd(year, thanksgiving.getMonth(), thanksgiving.getDate()), location: "Canada" });
  // Remembrance Day
  events.push({ id: `holiday-${year}-11-11`, title: "Remembrance Day", description: "Federal holiday", date: ymd(year, 10, 11), location: "Canada" });
  // Christmas Day
  events.push({ id: `holiday-${year}-12-25`, title: "Christmas Day", description: "Federal holiday", date: ymd(year, 11, 25), location: "Canada" });
  // Boxing Day
  events.push({ id: `holiday-${year}-12-26`, title: "Boxing Day", description: "Federal holiday", date: ymd(year, 11, 26), location: "Canada" });
  return events;
}
...
export function generateProvincialHolidays(year: number, province: ProvinceCode | null): Event[] {
  ...
  events.push({
    ...
    title: province === "MB" ? "Terry Fox Day" : province === "BC" ? "BC Day" : province === "NS" ? "Natal Day" : "Civic Holiday",
    description: "Provincial holiday",

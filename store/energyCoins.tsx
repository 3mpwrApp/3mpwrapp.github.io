...
export function EnergyCoinsProvider({ children }: { children: React.ReactNode }){
  ...
  React.useEffect(()=>{ (async()=>{ try{ const raw=await AsyncStorage.getItem(KEY); if(raw){ const d=JSON.parse(raw); if(d.date!==today){ setDate(today); setCoins(d.default||10); setSpent([]);} else { setDate(d.date); setCoins(d.coins); setSpent(d.spent||[]);} } }catch{} })(); },[]);
  const persist = async(next:{date:string;coins:number;spent:CoinsState['spent']})=>{ setDate(next.date); setCoins(next.coins); setSpent(next.spent); try{ await AsyncStorage.setItem(KEY, JSON.stringify(next)); }catch{} };

...
export type CategoryFilter = "all" | "work_financial" | "tools_downloads" | "emergency_crisis";

export function filterResources(items: Resource[], opts: { region: RegionFilter; category: CategoryFilter; query?: string }) {
  const q = (opts.query ?? "").trim().toLowerCase();
  let base = items;
  if (opts.category !== "all") base = base.filter((r) => r.category === opts.category);
  if (opts.region === "canada") base = base.filter((r) => r.scope === "canada");
  else if (opts.region !== "all") base = base.filter((r) => r.scope === "province" && r.province === opts.region);
  if (!q) return base;
...
export function presentProvinceCodes(items: Resource[]): string[] {
  ...
  for (const r of items) if (r.scope === "province" && r.province) set.add(r.province);

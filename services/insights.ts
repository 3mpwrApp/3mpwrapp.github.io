...
export async function buildSymptomSummary(): Promise<string> {
  const entries =
    (await getCachedJSON<SymptomEntry[]>("wellness_symptom_entries")) || [];
  ...
  const bullets = entries
    .slice(0, 10)
    .map(
      (e) => `• ${e.date}: pain ${e.pain ?? "?"}; impact: ${e.impact ?? "-"}`,
    );
  return [
...
export async function buildSleepSummary(): Promise<string> {
  const entries =
    (await getCachedJSON<SleepEntry[]>("wellness_sleep_entries")) || [];
  ...
  const bullets = entries
    .slice(0, 10)
    .map(
      (e) => `• ${e.date}: ${e.sleepHours ?? "?"}h; energy ${e.energy ?? "?"}`,
    );
  return [
...
export async function buildCombinedEvidenceSummary(): Promise<string> {
  const [sym, slp] = await Promise.all([
    buildSymptomSummary(),
    buildSleepSummary(),
  ]);

---
layout: default
title: Terms of Service
permalink: /terms/
description: The terms and conditions for using the 3mpwr website and app.
---


{%- include status-banner.html -%}

# 3mpwrApp — Terms of Service

**Last Updated: October 25, 2025**  
**Version: 3.0**

## IMPORTANT LEGAL DISCLAIMERS

**BY USING THIS APP, YOU AGREE TO THESE TERMS AND ALL DISCLAIMERS BELOW.**

---

## ⚠️ COMPREHENSIVE DISCLAIMERS & LIABILITY LIMITATIONS

### 1. **NOT MEDICAL ADVICE**

**3mpwrApp IS NOT A MEDICAL DEVICE AND DOES NOT PROVIDE MEDICAL ADVICE.**

- This app provides general wellness information, symptom tracking tools, and educational resources only
- **NO CONTENT IN THIS APP CONSTITUTES MEDICAL ADVICE, DIAGNOSIS, OR TREATMENT**
- The app is not a substitute for professional medical care or emergency services
- **DO NOT RELY ON THIS APP FOR MEDICAL DECISIONS**
- Always consult qualified healthcare providers for medical concerns
- **IN MEDICAL EMERGENCIES, CALL 911 OR YOUR LOCAL EMERGENCY SERVICES IMMEDIATELY**
- We are not licensed medical professionals and assume no responsibility for health outcomes

**Wellness Features Disclaimer:**
- Symptom trackers, mood logs, pain forecasts, sleep tracking, and exercise guides are informational tools only
- No feature in this app can diagnose, treat, cure, or prevent any medical condition
- Individual results vary; features may not be suitable for all users
- Consult your doctor before starting any wellness, exercise, or nutrition program

---

### 2. **NOT LEGAL ADVICE**

**3mpwrApp DOES NOT PROVIDE LEGAL ADVICE AND DOES NOT CREATE AN ATTORNEY-CLIENT RELATIONSHIP.**

- This app provides general legal information, document templates, and educational resources only
- **NO CONTENT IN THIS APP CONSTITUTES LEGAL ADVICE**
- Laws, regulations, and procedures vary significantly by jurisdiction, province, state, and country
- Legal information provided may be incomplete, outdated, or not applicable to your situation
- **DO NOT RELY ON THIS APP FOR LEGAL DECISIONS**
- Always consult a licensed attorney in your jurisdiction for legal advice specific to your situation
- We are not licensed attorneys and assume no responsibility for legal outcomes

**Legal Resources Disclaimer:**
- Form generators, letter templates, rights checklists, and AI legal assistants are educational tools only
- Templates may require customization for your specific situation
- Deadlines, procedures, and requirements change frequently—verify with official sources
- **Filing errors or missed deadlines can have serious legal consequences**
- Professional legal review is strongly recommended before submitting any legal documents

**Advocate Directory Disclaimer:**
- The advocate directory and lawyer finder provide contact information only
- We do not endorse, guarantee, or verify the qualifications of listed professionals
- We are not responsible for the quality of services provided by third-party advocates or attorneys
- Users must independently verify credentials, licensing, and suitability

---

### 3. **NOT FINANCIAL OR TAX ADVICE**

**3mpwrApp DOES NOT PROVIDE FINANCIAL, INVESTMENT, OR TAX ADVICE.**

- Information about disability benefits, financial assistance, and budgeting is general information only
- **NO CONTENT IN THIS APP CONSTITUTES FINANCIAL, TAX, OR INVESTMENT ADVICE**
- Financial situations are unique; what works for others may not work for you
- Tax laws and benefit regulations vary by jurisdiction and change frequently
- **DO NOT RELY ON THIS APP FOR FINANCIAL OR TAX DECISIONS**
- Consult qualified financial advisors, tax professionals, or benefits specialists for guidance

---

### 4. **AI-GENERATED CONTENT DISCLAIMER**

**AI-GENERATED CONTENT MAY CONTAIN ERRORS, INACCURACIES, OR OUTDATED INFORMATION.**

- This app uses artificial intelligence to assist with document generation, case interpretation, translation, and other features
- AI models can produce incorrect, incomplete, biased, or inappropriate content
- **ALWAYS VERIFY AI-GENERATED CONTENT WITH QUALIFIED PROFESSIONALS**
- AI should not replace professional medical, legal, or financial judgment
- We are not responsible for decisions made based on AI-generated content
- AI features are tools only—final responsibility for accuracy and decisions rests with you

**AI Limitations:**
- AI does not understand context, emotions, or individual circumstances like a human professional
- AI models have knowledge cutoffs and may not reflect current laws, medical standards, or best practices
- AI translations may not capture cultural nuances or legal terminology accurately
- AI-generated legal documents may not comply with specific jurisdictional requirements

---

### 5. **CRISIS & EMERGENCY SERVICES DISCLAIMER**

**THIS APP IS NOT A SUBSTITUTE FOR EMERGENCY SERVICES OR CRISIS INTERVENTION.**

- **IF YOU OR SOMEONE ELSE IS IN IMMEDIATE DANGER, CALL 911 OR YOUR LOCAL EMERGENCY SERVICES IMMEDIATELY**
- This app cannot detect, prevent, or respond to emergencies
- Crisis resource listings are provided for informational purposes only
- **DO NOT USE THIS APP AS A CRISIS HOTLINE OR EMERGENCY COMMUNICATION TOOL**
- The panic button and safe landing page are support tools only—they do not alert emergency services
- We are not responsible for delays, failures, or consequences related to emergency situations

**Crisis Resources (Canada):**
- **Canada Suicide Prevention Service:** Call [1-833-456-4566](tel:1-833-456-4566) or text [45645](sms:45645) (24/7)
- **Emergency:** Call 911 or your local emergency number
- **Canadian Domestic Violence Hotline:** [1-800-363-9010](tel:1-800-363-9010) (24/7)
- **[Complete crisis resources →](/crisis-resources)**

---

### 6. **NO WARRANTY & "AS IS" BASIS**

**THIS APP IS PROVIDED "AS IS" WITHOUT ANY WARRANTY OF ANY KIND.**

- We make no guarantees about app availability, functionality, accuracy, or fitness for any particular purpose
- Features may change, break, or be discontinued at any time without notice
- **WE DO NOT WARRANT THAT:**
  - The app will be error-free, secure, or uninterrupted
  - Content is accurate, complete, or current
  - Results will meet your expectations
  - Data will be preserved, backed up, or recoverable

**Technical Limitations:**
- The app depends on third-party services (Firebase, Expo, APIs) that may fail or change
- Internet connectivity issues may affect functionality
- Device compatibility varies; not all features work on all devices
- We are not responsible for data loss, device issues, or technical problems

---

### 7. **LIMITATION OF LIABILITY**

**TO THE MAXIMUM EXTENT PERMITTED BY LAW, WE DISCLAIM ALL LIABILITY FOR ANY DAMAGES RESULTING FROM YOUR USE OF THIS APP.**

**You use this app entirely at your own risk. We are not liable for:**

- Medical outcomes, health deterioration, or injury resulting from reliance on app content
- Legal consequences, missed deadlines, denied claims, or adverse legal outcomes
- Financial losses, denied benefits, tax penalties, or economic harm
- Data loss, breaches, or unauthorized access to your information
- AI errors, inaccuracies, or inappropriate content
- Emotional distress, mental health crises, or psychological harm
- Decisions made or actions taken based on app content or features
- Third-party services, providers, or resources accessed through the app
- Technical failures, bugs, crashes, or service interruptions
- Any indirect, incidental, consequential, punitive, or special damages

**This limitation applies regardless of:**
- Whether we were aware of the possibility of such damages
- The legal theory (contract, tort, negligence, strict liability, etc.)
- Whether the app was used as intended or for an unintended purpose

**Maximum Liability:**
- If any liability is found despite these limitations, our total liability shall not exceed $100 USD or the amount you paid for the app (whichever is less)
- For free users, our liability is limited to $0

---

### 8. **USER RESPONSIBILITY & INFORMED CONSENT**

**BY USING THIS APP, YOU ACKNOWLEDGE AND AGREE THAT:**

- You are solely responsible for your decisions, actions, and use of app content
- You will not rely on this app as a substitute for professional medical, legal, or financial advice
- You will independently verify all information with qualified professionals before making decisions
- You understand the limitations, disclaimers, and risks described in these Terms
- You will use the app in compliance with all applicable laws and regulations
- You will not use the app for illegal, harmful, or abusive purposes
- You are capable of giving informed consent (18+ or have guardian consent)
- You have read, understood, and accepted all disclaimers and limitations in these Terms

---

## 🔐 PRIVACY & DATA OWNERSHIP

### **100% User Data Ownership Guarantee**

**Your data belongs entirely to you.**

- All data created in 3mpwrApp is 100% owned by you
- We never claim ownership of your personal information
- Local-first, air-gapped architecture keeps data on your device
- Optional sync connects only to your chosen cloud services
- No tracking, analytics, or data collection without explicit consent
- See our **[Privacy Policy](/privacy/)** for complete details

### **Security & Encryption**

- AES-256 encryption for sensitive data
- Hardware-backed key storage
- Optional biometric authentication
- TLS 1.3 for network communications
- No data passes through our servers without your explicit consent

**[Read our Data Ownership Statement →](/data-ownership/)**

---

## 📋 SERVICE TERMS

### 1. Acceptance of Terms

By accessing or using 3mpwrApp's website, progressive web app, or services, you agree to comply with and be bound by these Terms of Service. If you do not agree to these terms, please do not use our Services.

### 2. Description of Service

3mpwrApp provides:

- **Community platform** for injured workers and persons with disabilities
- **Resources, tools, and information** about disability rights, benefits, and accessibility
- **Wellness tracking** for mood, energy, pain, sleep, and symptoms
- **Evidence Locker** for organizing important documents
- **Legal workflow automation** with letter templates and form generators
- **Advocacy resources** including rights checklists and government navigator
- **Cultural accessibility** with Indigenous language support and territorial acknowledgments
- **Connection** to community support and peer matching

### 3. Beta Testing & Current Status

**Current Status**: 3mpwrApp is in **Phase 1 - Closed Beta** testing.

**What this means**:
- Features may change without notice during beta development
- Bugs and errors may occur as we refine functionality
- Performance may vary across devices
- Service may be interrupted for updates and improvements
- Data persistence is provided, but backup is recommended
- Limited feature set compared to full release

**Beta Tester Responsibilities**:
- Provide constructive feedback on bugs and usability issues
- Use the app responsibly and ethically
- Respect community guidelines
- Understand that features are under active development
- Report security concerns immediately

### 4. User Accounts

**Account Creation**:
- You may need to create an account to access certain features
- You must provide accurate information
- You are responsible for maintaining account security
- You must be 13+ years old to create an account

**Account Security**:
- Keep your password confidential
- Notify us immediately of unauthorized access
- You are responsible for all activity on your account

### 5. User Conduct

**Prohibited Activities**:
- Harassment, bullying, threatening, or abusive behavior
- Posting illegal, harmful, or offensive content
- Impersonating others or providing false information
- Attempting to hack, exploit, or disrupt the service
- Spamming or unauthorized advertising
- Violating others' privacy or intellectual property rights
- Sharing dangerous medical misinformation
- Promoting conversion therapy or "curing" disabilities
- Coordinating illegal activities

**Community Guidelines**:
- Treat all users with respect and dignity
- Keep discussions relevant and constructive
- Protect sensitive personal information
- Report violations to moderators
- Respect identity-first and person-first language preferences
- Support trauma-informed communication

**[Read full Community Guidelines →](/community/guidelines/)**

### 6. Content and Intellectual Property

**Your Content**:
- You retain ownership of content you create
- You grant us a license to display and distribute your content
- You are responsible for your content's legality
- We may remove content that violates these terms

**Our Content**:
- 3mpwrApp content is protected by copyright
- You may not reproduce or distribute our content without permission
- Limited personal, non-commercial use is permitted

**Third-Party Content**:
- We may link to or include third-party content
- We are not responsible for third-party content accuracy
- Third-party sites have their own terms and privacy policies

### 7. Privacy and Data

**Data Collection**:
- See our **[Privacy Policy](/privacy/)** for comprehensive details
- We collect minimal data necessary to provide services
- We do not sell your personal information under any circumstances
- All data collection is transparent and user-controlled

**Data Security**:
- We implement industry-standard security measures
- AES-256 encryption, hardware-backed keys, TLS 1.3 network security
- Regular security audits and OWASP Mobile Top 10 compliance
- No system is 100% secure—users share responsibility for account security

**Data Retention**:
- Local device data persists until you delete the app or clear data
- Account and community content retained while account is active
- You may request data deletion at any time
- Some data may be retained for legal compliance

**Your Data Rights**:
- **Access:** Request to view your data
- **Correction:** Fix inaccurate information
- **Deletion:** Remove your data from our systems
- **Portability:** Export your data in standard formats
- **Opt-out:** Disable analytics and optional data collection

**[Learn more about Privacy Controls →](/privacy-controls/)**

### 8. Disclaimers

**No Warranties**:
- Services provided "AS IS" without warranties of any kind
- No guarantee of accuracy, reliability, or availability
- No warranty of fitness for a particular purpose
- Features may change, break, or be discontinued without notice

**Not Medical Advice**:
- Wellness information is educational only
- Not a substitute for professional medical care
- Consult qualified healthcare providers for medical concerns
- **IN EMERGENCIES, CALL 911 IMMEDIATELY**

**Not Legal Advice**:
- Legal information is educational only
- Not a substitute for licensed attorney consultation
- Laws vary by jurisdiction—verify with local professionals
- **Filing errors can have serious legal consequences**

**Not Financial Advice**:
- Financial and benefits information is general guidance only
- Not a substitute for qualified financial or tax advisors
- Individual situations vary—consult professionals

**AI Content Disclaimer**:
- AI-generated content may contain errors or inaccuracies
- Always verify AI output with qualified professionals
- AI is a tool only—you bear final responsibility

**Third-Party Services**:
- We are not responsible for third-party services
- Use third-party services at your own risk
- Third-party sites have their own terms and policies

**[Read all Disclaimers →](/legal/disclaimers/)**

### 9. Limitation of Liability

**To the maximum extent permitted by law**:
- We are not liable for indirect, incidental, or consequential damages
- Our liability is limited to the amount you paid us (if any)
- Some jurisdictions don't allow these limitations

**Indemnification**:
- You agree to indemnify us against claims arising from your use
- This includes costs of defending against claims

### 10. Termination

**We may terminate or suspend access**:
- For violations of these terms
- For illegal activity
- At our discretion with or without notice

**Upon termination**:
- Your right to use services ceases immediately
- We may delete your account and content
- Provisions that should survive termination will continue

### 11. Changes to Terms

**We may modify these terms**:
- Changes will be posted on this page
- Material changes will be communicated via email or notice
- Continued use after changes constitutes acceptance
- You should review terms periodically

### 12. Governing Law and Disputes

**Governing Law**:
- These terms are governed by the laws of Canada
- Disputes subject to jurisdiction of Canadian courts

**Dispute Resolution**:
- Attempt to resolve disputes informally first
- Formal disputes may require mediation or arbitration
- Some claims may be brought in small claims court

### 13. Accessibility

**Our Commitment**:
- We strive for WCAG 2.1 Level AAA compliance
- Cognitive accessibility features for all disability types
- Screen reader optimization, high contrast modes, and dyslexia-friendly fonts
- Motor disability accommodations with voice navigation
- Cultural accessibility with Indigenous language support
- See our **[Accessibility](/accessibility/)** page for comprehensive details
- Contact us to report accessibility issues—we respond within 48 hours

**[Learn more about Accessibility Features →](/accessibility/)**

### 14. International Users

**Global Access**:
- Services available globally where permitted
- You must comply with local laws
- We make no claims about appropriateness in your jurisdiction

### 15. Children's Privacy

**Age Restrictions**:
- Services not intended for children under 13
- We do not knowingly collect data from children under 13
- Parents: contact us if your child has provided information

### 16. Miscellaneous

**Entire Agreement**:
- These terms constitute the entire agreement
- Supersedes all prior agreements

**Severability**:
- If any provision is invalid, others remain in effect

**No Waiver**:
- Failure to enforce a right doesn't waive that right

**Assignment**:
- You may not assign these terms
- We may assign to affiliates or successors

**Force Majeure**:
- We are not liable for delays due to circumstances beyond our control

### 17. Contact Information

**For questions about these terms**:

**Email**: [empowrapp08162025@gmail.com](mailto:empowrapp08162025@gmail.com)  
**Response Time**: 24-48 hours  
**Website**: [https://3mpwrapp.pages.dev](https://3mpwrapp.pages.dev)  
**Organization**: 3mpwrApp  
**Location**: Toronto, ON, Canada

**For legal matters**:
Please send correspondence to the email above with "Legal" in the subject line.

**For privacy concerns**:
See our **[Privacy Policy](/privacy/)** or email with "Privacy" in the subject line.

**For accessibility issues**:
See our **[Accessibility](/accessibility/)** page or email with "Accessibility" in the subject line.

---

## 📄 Related Documents

- **[Privacy Policy](/privacy/)** - How we handle your data
- **[Data Ownership Statement](/data-ownership/)** - Your 100% data sovereignty
- **[Privacy Controls](/privacy-controls/)** - Manage your privacy settings
- **[Community Guidelines](/community/guidelines/)** - Community standards and moderation
- **[Accessibility](/accessibility/)** - Our accessibility features and compliance
- **[Legal Disclaimers](/legal/disclaimers/)** - All disclaimers in one place

---

**Language**: This agreement is available in English, French, Spanish, Arabic, Chinese, and Punjabi. In case of discrepancy, the English version prevails.

**Version**: 3.0  
**Effective**: October 25, 2025  
**Current Phase**: Closed Beta Testing

---

**© 2025 3mpwrApp. All rights reserved.**

{%- include page-feedback.html -%}

...
for (const file of jsonFiles) {
  ...
  try { readJSON(file); } catch (e) { errors.push(e.message); }
}
...
try {
  ...
  const fg = app?.expo?.android?.adaptiveIcon?.foregroundImage && path.join(base, app.expo.android.adaptiveIcon.foregroundImage);
  if (fg && !exists(fg)) warns.push(`Missing android adaptiveIcon.foregroundImage at ${app.expo.android.adaptiveIcon.foregroundImage}`);
  const fav = app?.expo?.web?.favicon && path.join(base, app.expo.web.favicon);
  if (fav && !exists(fav)) warns.push(`Missing web favicon at ${app.expo.web.favicon}`);
} catch (e) { warns.push(`app.json asset check warning: ${e.message}`); }
...
try {
  ...
  if (base && !/^https?:\/\//.test(base)) warns.push('EXPO_PUBLIC_LLM_BASE should be http(s) URL');
} catch {}
...
function realpathCaseSensitive(p) {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    const targetDir = i === parts.length - 1 ? path.dirname(path.join(base, seg)) : path.join(cur, '');
    try {
      const list = fs.readdirSync(path.dirname(path.join(cur, seg)));
      const found = list.find((name) => name.toLowerCase() === path.basename(seg).toLowerCase());
...
try {
  ...
  roots.forEach((r) => { const p = path.join(base, r); if (exists(p)) walk(p); });
  const importRE = /import\s+[^'"`]*from\s+['"](\.\.?\/[^'"`]+)['"];?|require\(\s*['"](\.\.?\/[^'"`]+)['"]\s*\)/g;
  for (const f of files.slice(0, 800)) { // limit to avoid long scans
    ...
    while ((m = importRE.exec(text))) {
      ...
      if (real && path.basename(real) !== path.basename(found)) {
        warns.push(`Import case differs from FS: ${path.relative(base, f)} -> ${rel}`);
      }
    }
  }
} catch (e) { warns.push(`Import case check warning: ${e.message}`); }
...
} catch (e) { warns.push(`Locale validation warning: ${e.message}`); }

// Ensure router layouts exist
[
  path.join(__dirname, '..', 'app', '_layout.tsx'),
  path.join(__dirname, '..', 'app', '(tabs)', '_layout.tsx'),
].forEach((p) => { if (!exists(p)) errors.push(`Missing router layout: ${p}`); });

#!/usr/bin/env node
/**
 * SEED ALL EVENTS TO FIREBASE & CLOUDFLARE
 * 
 ...
 * 
 * Usage: node scripts/seed-all-events.mjs
 */
...
function generateCanadianHolidays(year) {
  ...
  const labourDay = nthWeekdayOfMonth(year, 8, 1, 1); // September = month 8
  holidays.push({ id: `holiday-${year}-labour-day`, title: 'Labour Day', date: formatDate(labourDay), category: 'holiday', province: 'all' });
  
  // Thanksgiving (second Monday of October)
  const thanksgiving = nthWeekdayOfMonth(year, 9, 1, 2); // October = month 9
  holidays.push({ id: `holiday-${year}-thanksgiving`, title: 'Thanksgiving', date: formatDate(thanksgiving), category: 'holiday', province: 'all' });
  
  // Provincial holidays
  holidays.push(
    { id: `holiday-${year}-family-day-ab`, title: 'Family Day', date: nthWeekdayOfMonth(year, 1, 1, 3), category: 'holiday', province: 'AB' },
    { id: `holiday-${year}-family-day-bc`, title: 'Family Day', date: nthWeekdayOfMonth(year, 1, 1, 3), category: 'holiday', province: 'BC' },
    { id: `holiday-${year}-family-day-on`, title: 'Family Day', date: nthWeekdayOfMonth(year, 1, 1, 3), category: 'holiday', province: 'ON' },
    { id: `holiday-${year}-islander-day`, title: 'Islander Day', date: nthWeekdayOfMonth(year, 1, 1, 3), category: 'holiday', province: 'PE' },
    { id: `holiday-${year}-louis-riel`, title: 'Louis Riel Day', date: nthWeekdayOfMonth(year, 1, 1, 3), category: 'holiday', province: 'MB' },
...
// ============================================================================
// USER-CREATED EVENTS (Sample - Replace with actual AsyncStorage data)
// ============================================================================
function getUserCreatedEvents() {
  // REPLACE THIS WITH YOUR ACTUAL USER EVENTS
  // You can copy them from AsyncStorage or manually add them here
  return [
    {
      id: `evt-user-${Date.now()}-1`,
      title: 'Community Accessibility Workshop',
      description: 'Join us for a workshop on accessibility rights and resources. ASL interpretation provided.',
      ...
      stepFree: true,
      sensorySpace: false,
      tags: ['accessibility', 'workshop', 'community'],
      category: 'community',
      organizer: '3mpwrApp',
      createdBy: 'user-123' // Replace with actual user ID
    },
    {
      id: `evt-user-${Date.now()}-2`,
      title: 'Virtual Support Group Meeting',
      description: 'Monthly virtual support group for people with disabilities.',
      ...
      stepFree: false,
      sensorySpace: false,
      tags: ['support', 'virtual', 'community'],
      category: 'community',
      organizer: '3mpwrApp',
      createdBy: 'user-123' // Replace with actual user ID
    }
...
function nthWeekdayOfMonth(year, monthIndex, weekday, n) {
  // If n is negative, count from end of month
  if (n < 0) {
...
async function main() {
  console.log('╔══════════════════════════════════════════════════════════════╗');
  console.log('║              🌍 SEED ALL EVENTS TO FIREBASE                ║');
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  console.log(`   ✅ ${userEvents.length} user-created events`);
  
  const allEvents = [...holidays, ...observances, ...healthAwareness, ...userEvents];
  console.log(`\n   📦 Total: ${allEvents.length} events to sync\n`);
  ...
  for (const event of allEvents) {
    ...
    if (success) {
      prodSuccess++;
      process.stdout.write(`\r   Progress: ${prodSuccess}/${allEvents.length}`);
    }
  }
  console.log(`\n   ✅ Synced ${prodSuccess}/${allEvents.length} events\n`);
  ...
  for (const event of allEvents) {
    ...
    if (success) {
      previewSuccess++;
      process.stdout.write(`\r   Progress: ${previewSuccess}/${allEvents.length}`);
    }
  }
  console.log(`\n   ✅ Synced ${previewSuccess}/${allEvents.length} events\n`);
  ...
  console.log(`   • ${healthAwareness.length} health awareness events`);
  console.log(`   • ${userEvents.length} user-created events`);
  console.log(`   • ${allEvents.length} total events synced\n`);

...
export function MotorAccessibilityProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        console.warn('[MotorAccessibility] Failed to load preferences:', error);
      } finally {
        setIsLoading(false);
      }
    })();
  ...
  const setPreferences = async (partial: Partial<MotorAccessibilityPreferences>) => {
    ...
    try {
      ...
      console.warn('[MotorAccessibility] Failed to save preferences:', error);
    }
  };
  ...
  const reset = async () => {
    ...
    try {
      ...
      console.warn('[MotorAccessibility] Failed to reset preferences:', error);
    }
  };
